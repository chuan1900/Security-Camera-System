...
int main(int argc, const char * argv[]) {
	...
	struct timespec time10, time11, temp1;
	struct timespec time20, time21, temp2;
	struct timespec time30, time31, temp3;
    ...
    while((read=getline(&s,&len,stdin))!=-1){
        ...
        if (flag ==1){
            if (e_num == 0){
				...
				clock_gettime(CLOCK_REALTIME, &time20);
                pthread_create(&thread_APP1,NULL,&approxVC1,NULL);
				pthread_join(thread_APP1,NULL);
				clock_gettime(CLOCK_REALTIME, &time21);
				
                clock_gettime(CLOCK_REALTIME, &time30);
				pthread_create(&thread_APP2,NULL,&approxVC2,NULL);
				pthread_join(thread_APP2,NULL);
				clock_gettime(CLOCK_REALTIME, &time31);
                
                /*temp2 = diff(time20, time21);
                printf("APP1: %4ld.%09ld\n", temp2.tv_sec, temp2.tv_nsec);
                temp3 = diff(time30, time31);
                printf("APP2: %4ld.%09ld\n", temp3.tv_sec, temp3.tv_nsec);*/
				...
				temp1 = diff(time10, time11);
				printf("SAT: %4ld.%09ld\n", temp1.tv_sec, temp1.tv_nsec);
				temp2 = diff(time20, time21);
				printf("APP1: %4ld.%09ld\n", temp2.tv_sec, temp2.tv_nsec);
				temp3 = diff(time30, time31);
				printf("APP2: %4ld.%09ld\n", temp3.tv_sec, temp3.tv_nsec);
